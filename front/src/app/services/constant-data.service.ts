...
export class ConstantDataService {
  ...
  constructor(private http: HttpClient, private store: Store, private authService: AuthService) {
    ...
    const constantDataString = localStorage.getItem('constant-data');
    if (constantDataString) {
      this.formatConstantData(JSON.parse(constantDataString) as ConstantDataDTO);
    } else {
  ...
  private loadConstantData() {
    ...
    .subscribe((data: ConstantDataDTO) => {
      localStorage.setItem('constant-data', JSON.stringify(data));
      this.formatConstantData(data);
  ...
  formatConstantData(data: ConstantDataDTO) {
    // technologies
